...
class WassersteinGAN(object):
    ...
    def __init__(self,g_net,inputs,gt,alpha):
        ...
        #生成模型
        self.x_mis, self.mid, self.x_concat = self.g_net(self.z)

        
        #CD值loss的东东
        self.loss_fine = 0.5 * chamfer(self.x_mis, self.x) + 0.5 * chamfer(self.x_concat, self.x)
...
def fileSave(step,complete):
    ...
    for k in range(4):
        f = open('data/result10/bookcase'+str(k)+'_break'+str(step)+'.pcd', 'a')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lmdb_train', default='data/detail4096/detail.lmdb')
    parser.add_argument('--log_dir', default='log/detail_4096')
    parser.add_argument('--model_type', default='detail_4096')
    parser.add_argument('--batch_size', type=int, default=4)
    parser.add_argument('--num_input_points', type=int, default=3584)
