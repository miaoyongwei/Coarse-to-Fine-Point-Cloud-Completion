...
class WassersteinGAN(object):
    ...
    def __init__(self,g_net,inputs,gt,alpha):
        ...
        #生成模型
        self.x_saum, self.x_encoder_decoder, self.x_concat = self.g_net(self.z)

        
        #CD值loss的东东0.3*chamfer(self.x_encoder_decoder, self.x)+ 
        self.loss_fine = 0.5 * chamfer(self.x_saum, self.x) + 0.5 * chamfer(self.x_concat, self.x)
...
def fileSave(step,complete):
    ...
    for k in range(4):
        f = open('data/result20/saum'+str(k)+'_break'+str(step)+'.pcd', 'a')
...
def fileSave1(step,complete):

    for k in range(4):
        f = open('data/result20/encoder_decoder'+str(k)+'_break'+str(step)+'.pcd', 'a')
        f.writelines("# .PCD v0.7 - Point Cloud Data file format")
        f.writelines("\n")
        f.writelines("VERSION 0.7")
        f.writelines("\n")
        ...
        f.writelines("COUNT 1 1 1")
        f.writelines("\n")
        f.writelines("WIDTH 1936")
        f.writelines("\n")
        f.writelines("HEIGHT 1")
        f.writelines("\n")
        f.writelines("VIEWPOINT 0 0 0 1 0 0 0")
        f.writelines("\n")
        f.writelines("POINTS 1936")
        f.writelines("\n")
        f.writelines("DATA ascii")
        f.writelines("\n")
...
def fileSave2(step,complete):

    for k in range(4):
        f = open('data/result20/concat'+str(k)+'_break'+str(step)+'.pcd', 'a')
        f.writelines("# .PCD v0.7 - Point Cloud Data file format")
        f.writelines("\n")
        f.writelines("VERSION 0.7")
        f.writelines("\n")
        ...
        f.writelines("COUNT 1 1 1")
        f.writelines("\n")
        f.writelines("WIDTH 8080")
        f.writelines("\n")
        f.writelines("HEIGHT 1")
        f.writelines("\n")
        f.writelines("VIEWPOINT 0 0 0 1 0 0 0")
        f.writelines("\n")
        f.writelines("POINTS 8080")
        f.writelines("\n")
        f.writelines("DATA ascii")
        f.writelines("\n")
    ...
    try:
        while not coord.should_stop():
            ...
            if step % 300000 == 0:
                ...
                saver.save(sess, os.path.join(args.log_dir, 'model'), step)
            if step % 10000 == 0:
                complete1 = sess.run(wgan.x_saum)
                complete2 = sess.run(wgan.x_encoder_decoder)
                complete3 = sess.run(wgan.x_concat)
                fileSave(step,complete1)
                fileSave1(step,complete2)
                fileSave2(step,complete3)
            #如果步数大于最大步数
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lmdb_train', default='data/2048.lmdb')
    parser.add_argument('--log_dir', default='log/saum6')
    parser.add_argument('--model_type', default='saum')
