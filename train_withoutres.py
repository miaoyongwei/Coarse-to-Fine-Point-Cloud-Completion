...
class WassersteinGAN(object):
    ...
    def __init__(self,g_net,inputs,gt,alpha):
        ...
        #生成模型
        self.x_miss, self.x_concat = self.g_net(self.z)

        
        #CD值loss的东东
        self.loss_fine = 0.5 * earth_mover(self.x_miss, self.x) + 0.5 * (earth_mover(self.x_concat, self.x) + chamfer(self.x_concat, self.x))
...
def fileSave(step,complete):
    ...
    for k in range(4):
        f = open('data/result22/bookcase'+str(k)+'_break'+str(step)+'.pcd', 'a')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='log/concat_withoutres')
    parser.add_argument('--model_type', default='concat_withoutres')
    ...
    parser.add_argument('--max_step', type=int, default=250000)
