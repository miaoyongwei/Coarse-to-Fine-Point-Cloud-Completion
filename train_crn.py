...
class WassersteinGAN(object):
    ...
    def __init__(self,g_net,inputs,gt,alpha):
        ...
        #生成模型
        self.x_coarse, self.x_fine = self.g_net(self.z)

        
        #CD值loss的东东0.3*chamfer(self.x_encoder_decoder, self.x)+ 
        self.loss_fine = 0.5 * chamfer(self.x_coarse, self.x) + chamfer(self.x_fine, self.x)
...
def fileSave(step,complete):
    ...
    for k in range(4):
        f = open('data/result21/coarse'+str(k)+'_break'+str(step)+'.pcd', 'a')
...
def fileSave1(step,complete):
    ...
    for k in range(4):
        f = open('data/result21/fine'+str(k)+'_break'+str(step)+'.pcd', 'a')
        f.writelines("# .PCD v0.7 - Point Cloud Data file format")
        f.writelines("\n")
        f.writelines("VERSION 0.7")
        f.writelines("\n")
    ...
    try:
        while not coord.should_stop():
            ...
            if step % 300000 == 0:
                ...
                saver.save(sess, os.path.join(args.log_dir, 'model'), step)
            if step % 10000 == 0:
                complete1 = sess.run(wgan.x_coarse)
                complete2 = sess.run(wgan.x_fine)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='log/crn1')
    parser.add_argument('--model_type', default='crn')
