...
class WassersteinGAN(object):
    ...
    def __init__(self,g_net,inputs,gt):
        ...
        #生成模型
        self.x_, self.concat= self.g_net(self.z)
        
        #CD值loss的东东
        self.loss_fine = 0.5 * chamfer(self.x_, self.x) + 0.5 * chamfer(self.concat, self.x)

def fileSave(step,complete):

    for k in range(4):
        f = open('data/result26/bookcase'+str(k)+'_break'+str(step)+'.pcd', 'a')
...
def train(args):  
    ...
    wgan = WassersteinGAN(g_net,inputs,gts)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='log/atlas')
    parser.add_argument('--model_type', default='atlas')
