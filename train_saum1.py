...
class WassersteinGAN(object):
    ...
    def __init__(self,g_net,inputs,gt,alpha):
        ...
        #CD值loss的东东 
        self.loss_fine = 0.5 * chamfer(self.x_saum, self.x) + 0.1 * chamfer(self.x_concat, self.x) + 0.4 * chamfer(self.x_encoder_decoder, self.x)
...
def fileSave(step,complete):
    ...
    for k in range(4):
        f = open('data/result25/saum'+str(k)+'_break'+str(step)+'.pcd', 'a')
...
def fileSave1(step,complete):
    ...
    for k in range(4):
        f = open('data/result25/encoder_decoder'+str(k)+'_break'+str(step)+'.pcd', 'a')
        f.writelines("# .PCD v0.7 - Point Cloud Data file format")
        f.writelines("\n")
        f.writelines("VERSION 0.7")
        f.writelines("\n")
...
def fileSave2(step,complete):
    ...
    for k in range(4):
        f = open('data/result25/concat'+str(k)+'_break'+str(step)+'.pcd', 'a')
        f.writelines("# .PCD v0.7 - Point Cloud Data file format")
        f.writelines("\n")
        f.writelines("VERSION 0.7")
        f.writelines("\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='log/saum_model/saum4')
